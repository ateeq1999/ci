...
fn configures_app_module_installs_with_npm_and_adds_redis_url() {
    ...
    assert!(app_module.contains("import { BullModule } from '@nestjs/bullmq';"));
    assert!(app_module.contains("import IORedis from 'ioredis';"));
    assert!(app_module.contains("BullModule.forRoot({"));
    assert!(app_module.contains("new IORedis(process.env.REDIS_URL"));
    assert!(app_module.contains("maxRetriesPerRequest: null"));
...
fn running_twice_does_not_duplicate_the_module() {
    ...
    let app_module = read(&ctx, "src/app.module.ts");
    assert_eq!(app_module.matches("BullModule.forRoot(").count(), 1);
    assert_eq!(app_module.matches("import { BullModule }").count(), 1);
...
fn reports_already_configured_on_second_run() {
    ...
    assert!(
        messages
            .iter()
            .any(|m| m == "success: Queues were already configured")
    );
}

#[test]
fn sharing_redis_url_with_cache_does_not_duplicate_the_env_line() {
    let (ctx, _calls) = ctx_from_real_init_output();
    let bus = crate::commands::add::listeners::bus(&ctx);
    ...
    let app_module = read(&ctx, "src/app.module.ts");
    assert!(app_module.contains("import { CacheModule }"));
    assert!(app_module.contains("import { BullModule }"));
    let cache_idx = app_module.find("import { CacheModule }").unwrap();
    let queue_idx = app_module.find("import { BullModule }").unwrap();
