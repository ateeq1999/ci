...
fn configures_app_module_and_installs_with_npm_by_default() {
    ...
    assert!(app_module.contains("import { EventEmitterModule } from '@nestjs/event-emitter';"));
    assert!(app_module.contains("EventEmitterModule.forRoot(),"));
...
fn running_twice_does_not_duplicate_the_module() {
    ...
    let app_module = read(&ctx, "src/app.module.ts");
    assert_eq!(
        app_module.matches("EventEmitterModule.forRoot()").count(),
        1
    );
    assert_eq!(
        app_module.matches("import { EventEmitterModule }").count(),
        1
    );
}
...
fn reports_already_configured_on_second_run() {
    ...
    assert!(
        messages
            .iter()
            .any(|m| m == "success: Events were already configured")
    );
}

#[test]
fn stacks_after_schedule_instead_of_colliding_on_the_same_anchor() {
    let (ctx, _calls) = ctx_from_real_init_output();
    let bus = crate::commands::add::listeners::bus(&ctx, root());

    crate::commands::add::schedule::run(&ctx, root(), &bus).unwrap();
    run(&ctx, root(), &bus).unwrap();
    ...
    let events_idx = app_module.find("import { EventEmitterModule }").unwrap();
    assert!(schedule_idx < events_idx);
}
